...
const PaymentDialog = ({ reservation, tString }: PaymentDialogProps) => {
...
const MyReservations = () => {
  ...
  const filteredReservations = reservations.filter((r) => {
    ...
    if (filters.status) {
      ...
      if (
        statusNormalized !== reservationStatus &&
        statusNormalized !== reservationStatusLabel
      )
        return false;
    }
  ...
  const statuses = [
    ...
    {
      value: "Payment Refund Requested",
          ...
          <Card className="mb-8 animate-fade-in">
            ...
            <CardContent>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4 items-end">
                <div>
                  <Label>
                    {tString("myReservations.filters.projectLabel")}
                  </Label>
                  <Select
                    ...
                    <SelectTrigger>
                      <SelectValue
                        placeholder={tString(
                          "myReservations.filters.allProjects"
                        )}
                      />
                    </SelectTrigger>
                    <SelectContent>
                ...
                <Table>
                  <TableHeader>
                    <TableRow>
                      ...
                      <TableHead>
                        {tString("myReservations.table.formNo")}
                      </TableHead>
                      <TableHead>
                        {tString("myReservations.table.status")}
                      </TableHead>
                      <TableHead>
                        {tString("myReservations.table.unitType")}
                      </TableHead>
                      <TableHead>
                        {tString("myReservations.table.price")}
                      </TableHead>
                      <TableHead>
                        {tString("myReservations.table.modified")}
                      </TableHead>
                      <TableHead>
                        {tString("myReservations.table.city")}
                      </TableHead>
                      <TableHead>
                      ...
                      <TableRow
                        ...
                        <TableCell>
                          {reservation.paymentAttachments ? (
                            <div className="flex flex-col gap-1">
                              {reservation.paymentAttachments.map((file) => (
                                <Button
                                  key={file}
                                  variant="outline"
                                  size="sm"
                        ...
                        <TableCell>
                          <div className="flex flex-col gap-2">
                            {(reservation.status === "New" ||
                              reservation.status ===
                                "Reservation Payment Update Requested") && (
                              <PaymentDialog
                                reservation={reservation}
                                tString={tString}
                              />
